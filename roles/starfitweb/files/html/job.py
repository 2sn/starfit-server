...
def set_result_values(result, config):
    ...
    lolim = [
        I(x) for x in config.z_lolim if x in eval_elements and x not in config.z_exclude
    ]
    ...
    matched_elements = [
        I(x)
        for x in eval_elements
        if not ((x in config.z_exclude) or (x in config.z_lolim) or (x in upper_limits))
    ]
    ...
    included_upper_limits = [
        I(x) for x in upper_limits if x not in config.z_exclude and x in eval_elements
    ]
    config.included_upper_limits_string = ", ".join(
        x.Name() for x in included_upper_limits
    )
    config.included_upper_limits_string = compressed_ion_list(
        config.included_upper_limits_string
    )
    ...
    for db, l in zip(result.db, result.db_lab):
        lower_warn = list()
        for i in db.lower:
            if i.Z in matched_elements:
                lower_warn.append(i)
        exclude_warn = list()
        for i in db.exclude:
            if i.Z in included_upper_limits or i.Z in matched_elements or i.Z in lolim:
